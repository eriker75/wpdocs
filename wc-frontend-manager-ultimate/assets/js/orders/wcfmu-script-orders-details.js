jQuery(document).ready(function($) {
	...
	function downloadPDFInvoiceWCFMOrder(item) {
		if (wcfm_params.ajax_url.indexOf("?") != -1) {
			url = wcfm_params.ajax_url + '&action=wcfm_order_pdf_invoice&template_type=invoice&order_id='+item.data('orderid')+'&wcfm_ajax_nonce='+wcfm_params.wcfm_ajax_nonce;
		} else {
			url = wcfm_params.ajax_url + '?action=wcfm_order_pdf_invoice&template_type=invoice&order_id='+item.data('orderid')+'&wcfm_ajax_nonce='+wcfm_params.wcfm_ajax_nonce;
		}
	...
	function downloadPDFPackingSlipWCFMOrder(item) {
		if (wcfm_params.ajax_url.indexOf("?") != -1) {
			url = wcfm_params.ajax_url + '&action=wcfm_order_pdf_packing_slip&template_type=packing_slip&order_id='+item.data('orderid')+'&wcfm_ajax_nonce='+wcfm_params.wcfm_ajax_nonce;
		} else {
			url = wcfm_params.ajax_url + '?action=wcfm_order_pdf_packing_slip&template_type=packing-slip&order_id='+item.data('orderid')+'&wcfm_ajax_nonce='+wcfm_params.wcfm_ajax_nonce;
		}
		window.open(url, '_blank');
	}
	...
	$('#wcfm_add_order_note').click(function(event) {
		...
		var note = $('#add_order_note').val().trim();

		if( ! note ) {
			alert( wcfmu_orders_details_i18n.add_order_note_validation );
		} else {
			addWCFMOrderNote();
		}
