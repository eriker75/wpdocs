...
class WCFMu_License
{
    ...
    public static function license_inactive_notice()
    {
        ...
        <div id="message" class="error settings-error notice is-dismissible">
            <p><?php 
            // translators: 1) license tab url start 2) license tab url end
            printf(__('The WCFM - Ultimate License Key has not been activated, so the plugin is inactive! %1$sClick here%2$s to activate the license key and the plugin.', 'wc-frontend-manager-ultimate'), '<a href="'.esc_url(admin_url('admin.php?page=wcfm-license&tab='.str_replace('-', '_', esc_attr($WCFMu->token)).'_license')).'">', '</a>'); 
            ?></p>
