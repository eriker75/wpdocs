...
class WCFMu_WCAddons
{
    ...
    public function __construct()
    {
        ...
        if (apply_filters('wcfm_is_pref_wc_product_addon', true) && apply_filters('wcfm_is_allow_wc_product_addon', true)) {
            if (WCFMu_Dependencies::wcfm_wc_addons_active_check() || WCFMu_Dependencies::wcfm_wc_appointments_active_check()) {
                // WC Addons 5.0+ compatibility
                if ( ( defined('WC_PRODUCT_ADDONS_VERSION') && version_compare(WC_PRODUCT_ADDONS_VERSION, '5.0.0', '>') ) || ( defined('WC_APPOINTMENTS_VERSION') && version_compare(WC_APPOINTMENTS_VERSION, '4.15.0', '>') ) ) {
                    ...
                    add_action('wcfm_load_scripts', [ &$this, 'wcaddons_load_scripts_5' ], 90);
                    add_action('after_wcfm_load_scripts', [ &$this, 'wcaddons_load_scripts_5' ], 90);

                    // WC Product Addons Product Manage View
                    add_action('after_wcfm_products_manage_meta_save', [ &$this, 'wcaddons_wcfm_products_manage_meta_save_3' ], 200, 2);
                } else if (( defined('WC_PRODUCT_ADDONS_VERSION') && version_compare(WC_PRODUCT_ADDONS_VERSION, '3.0.0', '>=') ) || WCFMu_Dependencies::wcfm_wc_appointments_active_check()) {
     ...
     * WC Product Addons Scripts
     */
    public function wcaddons_load_scripts_5($end_point)
    {
        global $WCFM, $WCFMu;
        ...
        switch ($end_point) {
            case 'wcfm-products-manage':
                ...
                wp_register_script('woocommerce_product_addons', plugins_url('assets/js/admin/admin'.$suffix.'.js', WC_PRODUCT_ADDONS_MAIN_FILE), [ 'jquery' ], WC_PRODUCT_ADDONS_VERSION, true);
